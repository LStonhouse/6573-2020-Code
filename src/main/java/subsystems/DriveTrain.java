...
public class DriveTrain extends Subsystem {
    ...
    private final static double WHEELBASE = 30.5; // Length between the axis'
    private final static double TRACKWIDTH = 14; // length between two motors on same axis
    
    private final static double RADIUS = Math.sqrt(Math.pow(WHEELBASE, 2) + Math.pow(TRACKWIDTH, 2));

    private final static double TICKS_PER_360 = 4096; // How many ticks in a 360 rotation of the steering column

    private final static double PIDP = 0.0012;
    private final static double PIDI = .0018;
    private final static double PIDD = 0;

    // BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=DECLARATIONS
private AnalogInput frontLeftSteerEncoder;
private AnalogInput frontRightSteerEncoder;
private AnalogInput backLeftSteerEncoder;
private AnalogInput backRightSteerEncoder;
...
backRightSteerEncoder = new Encoder(8, 9, false, EncodingType.k4X);
...
frontLeftSteerEncoder = new AnalogInput(0);
frontLeftSteerEncoder.getVoltage();

frontRightSteerEncoder = new AnalogInput(1);
frontRightSteerEncoder.getVoltage();

backLeftSteerEncoder = new AnalogInput(2);
backLeftSteerEncoder.getVoltage();

backRightSteerEncoder = new AnalogInput(3);
backRightSteerEncoder.getVoltage();
...
backRightSteer.setInverted(false);
    ...
    public void drive(double rotation, double x, double y){
        ...
        ticks[0] = frontRightSteerEncoder.getVoltage();
        ticks[1] = frontLeftSteerEncoder.getVoltage();
        ticks[2] = backLeftSteerEncoder.getVoltage();
        ticks[3] = backRightSteerEncoder.getVoltage();

        for(int i = 0; i<4; i++){
            
            currentAngles[i] = ticks[i] / 5.0 * 360.0;
        }
